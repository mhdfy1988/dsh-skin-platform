...
当前正式版本：`0.1.0`；当前开发候选：`0.1.1-rc.1`。两者均适配 DeepSeek Harness `0.1.1-rc.2`，仅支持 Web 客户端。
...
| `dsh-skin-dream-journey` | “梦境仙游”瓷白云境、桃花暖粉、天青玉色与淡金描边的可爱国风皮肤 |

皮肤包只注册声明式数据和自己的静态资源，不直接查找或修改 Harness 内部 DOM。
...
- 每套皮肤的用户背景分别保存在当前 `<DSH_HOME>/skin-runtime/<skin-id>/user-background`，不进入设置文件或皮肤安装包；
- 官方主题 token 的可撤销覆盖；
...
- `/skin-assets/user/background/<skin-id>` 按皮肤同源上传、读取和移除；
...
pnpm.cmd dsh plugin --profile web add `
  D:\deepseek\dsh-skin-platform\artifacts\dsh-skin-runtime-0.1.1-rc.1.tgz `
  D:\deepseek\dsh-skin-platform\artifacts\dsh-skin-void-whisper-0.1.1-rc.1.tgz `
  D:\deepseek\dsh-skin-platform\artifacts\dsh-skin-dream-journey-0.1.1-rc.1.tgz
```
...
当前自动化覆盖声明式清单、固定颜色策略、light/dark token 成对值、稳定插槽着色、组合包声明、服务名注入、SVG 无脚本内容、PNG 字节签名、图片格式识别、有界读取和本机文件生命周期。

## 已验证的真实链路

- 三个 tgz 同时安装到隔离 Web profile；
- 共享运行时和两个皮肤包的 Host、Client 入口全部激活；
- “虚空低语”SVG 资源返回 `200 image/svg+xml`；
- 目录穿越请求返回 404；
- 设置页列出“虚空低语”和“梦境仙游”皮肤；
- 本机 PNG 上传后可预览并覆盖工作区背景；
- “虚空低语”设置本机背景后切换到“梦境仙游”，后者仍使用自己的包内背景，图片和显示参数不会跨皮肤串用；
...
- 用户背景仅支持当前 Web 配置档案中每套皮肤各一张本机图片，没有壁纸库、同步、导入导出或市场；
